...
export const restaurantAgent = new Agent({
    ...
    You will always reply in english, regardless the language the customer writes in.
    
    ## First message rule
    The chat UI already shows a welcome message before the customer types. You will see it in the conversation history.
    Never greet, welcome, or introduce yourself again.
    ...
    - Use weatherTool only when weather is relevant to a recommendation. Weather location: Vancouver, Canada`,
    ...
    tools: {
        ...
        getCartItems, getWaitTime, getNutritionInfo, getDietaryItems
    },
});
